...
class TestOutputManager:
    ...
    def test_deduplication(self, output_manager, mock_state, mock_context, tmp_path):
        ...
        vuln1 = output_manager.add_vulnerability(
            title="Duplicate Vuln", description="Test", state=mock_state, context=mock_context
        )
    ...
    def test_create_result(self, output_manager, tmp_path):
        ...
        # Create result
        result = output_manager.create_result(analysis_time=10.5, errors=["test error"])
...
class TestEnhancedVulnerability:
    ...
    def test_to_rich_dict(self):
        ...
        vuln = EnhancedVulnerability.from_detection(
            title="Test", description="Desc", state=mock_state, capture_full_state=False
        )
...
class TestAnalysisContextIntegration:
    ...
    def test_context_with_output_manager(self, tmp_path):
        ...
        with pytest.raises(Exception):
            context = AnalysisContext.create_for_driver(test_file, config, output_manager=output_manager)
    ...
    def test_add_vulnerability_with_manager(self):
        ...
        context = AnalysisContext(
            project=None, cfg=None, calling_convention=None, config=AnalysisConfig(), output_manager=output_manager
        )

        # Add vulnerability
        vuln_info = {"title": "Test", "description": "Test", "state": mock_state}
...
class TestOutputFormats:
    ...
    def test_vulnerability_summary(self):
        ...
        summary = VulnerabilitySummary(
            type="ARBITRARY_WRITE", severity="CRITICAL", count=3, ioctl_codes=["0x222000", "0x222004"]
        )
