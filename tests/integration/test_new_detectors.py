...
class TestFormatStringDetector:
    ...
    def test_format_string_vulnerabilities(self):
        ...
        assert len(format_string_vulns) > 0, f"No format string vulnerabilities found. Found: {[v['title'] for v in vulns]}"
    ...
    def test_format_string_functions(self):
        ...
        dbgprint_vulns = [v for v in vulns 
                         if "Format String" in v.get("title", "") 
                         and "DbgPrint" in str(v.get("parameters", {}).get("function", ""))]
...
class TestKernelPrimitiveDetector:
    ...
    def test_arbitrary_increment_decrement(self):
        ...
        inc_dec_vulns = [v for v in kernel_primitive_vulns 
                         if "increment" in v.get("title", "").lower() 
                         or "decrement" in v.get("title", "").lower()]
    ...
    def test_interlocked_operations(self):
        ...
        interlocked_vulns = [v for v in vulns 
                            if "Kernel Primitive" in v.get("title", "")
                            and "interlocked" in v.get("title", "").lower()]
...
class TestSymlinkAttackDetector:
    ...
    def test_toctou_detection(self):
        ...
        toctou_vulns = [v for v in symlink_vulns 
                       if "TOCTOU" in v.get("title", "") 
                       or "toctou" in str(v.get("parameters", {}).get("type", ""))]
    ...
    def test_unsafe_symlink_following(self):
        ...
        symlink_follow_vulns = [v for v in vulns 
                               if "Symlink Attack" in v.get("title", "")
                               and ("Unsafe" in v.get("title", "") 
                                    or "symlink_follow" in str(v.get("parameters", {}).get("type", "")))]
    ...
    def test_predictable_temp_files(self):
        ...
        temp_file_vulns = [v for v in vulns 
                          if "Symlink Attack" in v.get("title", "")
                          and ("Predictable" in v.get("title", "") 
    ...
    def test_file_creation_race(self):
        ...
        race_vulns = [v for v in vulns 
                     if "Symlink Attack" in v.get("title", "")
                     and ("Race" in v.get("title", "") 
                          or "creation_race" in str(v.get("parameters", {}).get("type", "")))]
...
class TestDetectorIntegration:
    ...
    def test_no_false_positives_on_safe_code(self):
        ...
        assert len(format_string_vulns) < 50, f"Too many format string vulnerabilities detected ({len(format_string_vulns)}), possible false positives"
