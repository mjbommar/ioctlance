...
def test_rtdashpt_basic_analysis(rtdashpt_driver_path: Path):
    ...
    # Verify basic info
    assert result.basic.ioctl_handler == "0x140007080", f"Expected handler at 0x140007080, got {result.basic.ioctl_handler}"
    
    # Should find at least some IOCTL codes
    assert len(result.basic.IoControlCodes) >= 3, f"Expected at least 3 IOCTL codes, found {len(result.basic.IoControlCodes)}"
