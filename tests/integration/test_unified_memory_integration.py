    ...
    if vulnerability_type == "double_free":
        ...
        code = bytes(
            [
                ...
                0x00,
                0x00,
                0x00,  # lea rcx, [pool_tag]
                0xFF,
                0x15,
                0x00,
                0x00,
                0x00,
                0x00,  # call ExAllocatePoolWithTag
                0x48,
                0x89,
                0xC3,  # mov rbx, rax (save ptr)
                0x48,
                0x89,
                0xD9,  # mov rcx, rbx
                0xFF,
                ...
                0x00,
                0x00,
                0x00,  # call ExFreePool (double free!)
                0xC3,  # ret
            ]
        )
        ...
        code = bytes(
            [
                ...
                0x48,
                0x8B,
                0x03,  # mov rax, [rbx] (use after free!)
                0xC3,  # ret
            ]
        )
        ...
        code = bytes(
            [
                0x48,
                0x89,
                0xC1,  # mov rcx, rax (object ptr)
                0xFF,
                0x15,
                0x00,
                0x00,
                0x00,
                0x00,  # call ObReferenceObject
                0x48,
                0x89,
                0xC1,  # mov rcx, rax
                0xFF,
                ...
                0x00,
                0x00,
                0x00,  # call ObDereferenceObject (underflow!)
                0xC3,  # ret
            ]
        )
    else:
...
class TestUnifiedMemoryIntegration:
    ...
    def create_project(self):
        ...
        def _create(vuln_type: str):
            ...
            project = angr.Project(
                binary_path, auto_load_libs=False, load_options={"main_opts": {"backend": "blob", "arch": "amd64"}}
            )
    ...
    def test_double_free_detection_symbolic(self, create_project, analysis_config):
        ...
        def hook_exallocatepool(state):
            ...
            # Track allocation
            detector._handle_allocation(state, "ExAllocatePoolWithTag", pool_type=0, size=0x100, tag="TEST")
            return ptr

        def hook_exfreepool(state):
            # Get pointer from RCX
            ptr = state.regs.rcx
            result = detector._handle_free(state, "ExFreePool", pool_ptr=ptr)
    ...
    def test_use_after_free_detection(self, create_project, analysis_config):
        ...
        detector._handle_allocation(state, "ExAllocatePoolWithTag", pool_type=0, size=0x100, tag="UAF")
        ...
        result = detector.check_state(state, "mem_read", address=alloc_addr, size=8)
    ...
    def test_pool_tag_tracking(self, analysis_config):
        ...
        for i, tag in enumerate(tags):
            detector._handle_allocation(state, "ExAllocatePoolWithTag", pool_type=0, size=0x100, tag=tag)
    ...
    def test_range_based_detection(self, analysis_config):
        ...
        detector.memory_regions[base_addr] = detector.memory_regions.get(
            base_addr
        ) or UnifiedMemoryDetector.MemoryRegion(address=base_addr, size=region_size, pool_tag="LARGE")
        ...
        result = detector.check_state(state, "mem_write", address=middle_addr, size=0x100)
    ...
    def test_windows_specific_patterns(self, analysis_config):
        ...
        detector.memory_regions[addr1] = UnifiedMemoryDetector.MemoryRegion(address=addr1, size=0x1000, pool_tag="CONT")
        detector.memory_regions[addr2] = UnifiedMemoryDetector.MemoryRegion(address=addr2, size=0x1000, pool_tag="HEAP")
