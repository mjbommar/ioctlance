...
def test_unconstrained_verification():
    ...
    vuln_stack = {
        ...
        "eval": {"IoControlCode": "0x220040", "SystemBuffer": "0x12345678", "InputBufferLength": "0x1000"},
        "others": {"severity": "CRITICAL", "type": "unconstrained_state"},
    }
...
def test_verification_config():
    ...
    config1 = AnalysisConfig(verification_enabled=False, verification_level="none")
    assert not config1.verification_enabled
    print("✓ Verification can be disabled")

    # Test with different levels
    config2 = AnalysisConfig(verification_enabled=True, verification_level="basic")
    assert config2.verification_level == "basic"
    print("✓ Basic verification level")

    config3 = AnalysisConfig(verification_enabled=True, verification_level="deep")
    assert config3.verification_level == "deep"
    print("✓ Deep verification level")

    # Test false positive filtering
    config4 = AnalysisConfig(verification_enabled=True, filter_false_positives=True, reclassify_vulnerabilities=True)
