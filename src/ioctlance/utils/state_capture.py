...
def capture_registers(state: SimState) -> RawRegisters:
    ...
    return RawRegisters(
        registers=registers,
        symbolic_registers=symbolic_regs
    )
...
def capture_symbolic_state(state: SimState) -> RawSymbolicState:
    ...
    taint_keywords = ['SystemBuffer', 'UserBuffer', 'Type3InputBuffer', 'IoControlCode', 
    ...
    return RawSymbolicState(
        ...
        symbolic_expressions=symbolic_exprs,
...
def capture_instruction_context(state: SimState) -> RawInstructionContext:
    ...
    return RawInstructionContext(
        address=addr,
        bytes=instr_bytes,
        disassembly=disasm,
        basic_block_start=bb_start
    )
...
def capture_concrete_inputs(state: SimState, context: AnalysisContext | None) -> RawConcreteInputs:
    ...
    if inputs.system_buffer_addr:
        try:
            ...
            buf_content = state.solver.eval(
                state.memory.load(inputs.system_buffer_addr, 64)
            )
...
def capture_state_globals(state: SimState) -> RawStateGlobals:
    ...
    try:
        ...
        skip_keys = {'device_object_addr', 'ioctl_handler', 'tainted_ProbeForRead',
                     'tainted_ProbeForWrite', 'tainted_MmIsAddressValid', 
                     'tainted_handles', 'tainted_eprocess', 'analysis_context'}
...
def capture_memory_snapshots(state: SimState, context: AnalysisContext | None, max_regions: int = 5) -> RawMemorySnapshot:
