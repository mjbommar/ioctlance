...
class UnconstrainedStateVerifier(VulnerabilityVerifier):
    ...
    def can_verify(self, vuln_info: dict[str, Any]) -> bool:
        ...
        return "unconstrained" in title or "controllable pc" in title or vuln_type == "unconstrained_state"
    ...
    def _check_stack_corruption(self, state: Any) -> dict[str, Any]:
        ...
        return {"evidence": evidence, "confidence": min(1.0, confidence)}
    ...
    def _handle_null_pointer(
        self, vuln_info: dict[str, Any], root_cause: dict[str, Any]
    ...
    def _handle_heap_overflow(
        ...
        enhanced["description"] = f"Heap buffer overflow detected. Evidence: {', '.join(root_cause['evidence'])}"
    ...
    def _handle_use_after_free(
        ...
        enhanced["description"] = f"Use-after-free vulnerability detected. Memory was accessed after being freed."
    ...
    def _handle_generic_corruption(
        self, vuln_info: dict[str, Any], root_cause: dict[str, Any]
    ) -> tuple[VerificationResult, dict[str, Any]]:
